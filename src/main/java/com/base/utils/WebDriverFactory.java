...
public abstract class WebDriverFactory {
  ...
  /**
   * click webelement with provided timeout.
   * 
   * @param element - element to click.
   * @param timeout - timeout to apply wait for.
   * @author apple
   */
  public static void clickWebElement(WebElement element, int timeout) {
    isElementVisible(element, timeout);
    if (!element.isEnabled()) {
      throw new WebDriverException(element.getText() + " is not clickable");
    }
    try {
  ...
  public static void sendKeys(WebElement element, String text, int timeout) {
    if (!element.isEnabled()) {
      throw new WebDriverException("WebElement " + element + " is not editabe as It is disabled");
    }
    try {
  ...
  public static void selectElementByVisibleText(WebElement element, String dropDownValue) {
    if (!element.isEnabled()) {
      throw new WebDriverException("WebElement " + element + " is not editabe as It is disabled");
    }
    try {
  ...
  public static void selectElementByIndex(WebElement element, int dropDownIndex) {
    if (!element.isEnabled()) {
      throw new WebDriverException("WebElement " + element + " is not editabe as It is disabled");
    }
    try {
   ...
   * 
   * @param image - Web element of image to be verified.
   * @return - true if image is broken else false.
   * @author simran.kumari
   */
   ...
   * 
   * @param webElement - web element to be right clicked on.
   * @author simran.kumari
   */
  public static void rightClickOnTheWebElement(WebElement webElement) {
    Actions actionsObj = new Actions(driver);
    actionsObj.moveToElement(webElement);
    actionsObj.contextClick(webElement).build().perform();
  }
   ...
   * 
   * @param timeout - timeout period.
   * @return - true if alert is displayed else false.
   * @author simran.kumari
   */
